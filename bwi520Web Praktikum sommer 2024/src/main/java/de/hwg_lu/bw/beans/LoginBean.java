...
public class LoginBean {
	...
	public boolean checkUseridPassword() throws SQLException {
		String sql = "SELECT  userid , password  from accounts where email = ? and password = ?";
	...
	public boolean checkUseridPassword2() throws SQLException {
		String sql = "SELECT username, userid, password from accounts where userid = ?";
		System.out.println(sql);
		Connection dbConn = new PostgreSQLAccess().getConnection();
		PreparedStatement prep = dbConn.prepareStatement(sql);
		prep.setString(1, this.email);
		...
		if (recordExists) {
			String myPassword = dbRes.getString("password").trim();
			 this.username    = dbRes.getString("username");
	...
	public String getAdminBtnn() throws SQLException {
		...
		String sql = "SELECT userid, password from accounts where email = ? and password = ? and admin= ?";
		System.out.println(sql);
		Connection dbConn = new PostgreSQLAccess().getConnection();
		PreparedStatement prep = dbConn.prepareStatement(sql);
